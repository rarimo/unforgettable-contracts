...
export = async (deployer: Deployer) => {
  ...
  const signatureRecoveryStrategy = await deployer.deployERC1967Proxy(
    SignatureRecoveryStrategy__factory,
    signatureRecoveryStrategyInitData,
    { name: "SignatureRecoveryStrategy" },
  );

  Reporter.reportContracts(["SignatureRecoveryStrategy", await signatureRecoveryStrategy.getAddress()]);
};
